...
Std_ReturnType SYSTICK_udtSynchSecondsDelay (uint16 u16Seconds)
{
	...
	if (SYSTICK_CLOCK_FREQ >= SYSTICK_MAX_CLOCK)
		return E_NOT_OK;
	...
	if(SYSTICK_u8IsInitialized == SYSTICK_INIT)
	{
	    ...
	    {
	    	/* !Comment: load 1 sec */
	    	SYSTICK -> LOAD.R = SYSTICK_CLOCK_FREQ;
...
{
	Std_ReturnType udtRetVal = E_NOT_OK;
	if (SYSTICK_CLOCK_FREQ/1000000 >= SYSTICK_MAX_CLOCK)
		return E_NOT_OK;
	...
	if(SYSTICK_u8IsInitialized == SYSTICK_INIT)
	{
	    ...
	    {
	    	/* !Comment: load 1 micro sec */
	    	SYSTICK -> LOAD.R = SYSTICK_CLOCK_FREQ/1000000;
...
Std_ReturnType SYSTICK_udtAsynchSecondsDelay (uint16 u16Seconds, void (*pvCallBack) (void))
{
	...
	if(SYSTICK_u8IsInitialized == SYSTICK_INIT)
	{
         ...
         *          	and also clears the COUNTFLAG bit in the STK_CTRL register to 0.        
	     */
	    SYSTICK -> VAL.R  = STD_OFF;
	    SYSTICK -> LOAD.R = STD_OFF;
        
	    /* !Comment: load 1 sec */
	    SYSTICK -> LOAD.R = SYSTICK_CLOCK_FREQ;
		
		/* !Comment: Enable interrupts */
		SYSTICK -> CTRL.B.TICKINT = STD_ON;
...
Std_ReturnType SYSTICK_udtAsynchMsecondsDelay (uint16 u16Seconds, void (*pvCallBack) (void))
{
	SYSTICK_u8SingleIndicator = 0u;
	SYSTICK_pvCallBack = pvCallBack;
    SYSTICK_u16GlobalSeconds = u16Seconds;
...
Std_ReturnType SYSTICK_udtAsynchUsecondsDelay (uint16 u16Seconds, void (*pvCallBack) (void))
{
	SYSTICK_u8SingleIndicator = 0u;
	SYSTICK_pvCallBack = pvCallBack;
    SYSTICK_u16GlobalSeconds = u16Seconds;
